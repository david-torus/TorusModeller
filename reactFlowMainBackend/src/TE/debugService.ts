...
export class DebugService {
  constructor(  private readonly jwtService: JwtService,  private readonly redisService : RedisService,private readonly commonService: CommonService, private readonly tecommonService: TeCommonService) {}
        ...
        try{
          ...
          if(mapper){
            if(mapper.pre.mapData){
              ...
              let mapperResult = await this.tecommonService.mapper(preRequest, preResponse,preMapData)
              m=1;
            ...
            if(mapper.pro.mapData){
              ...
              let mapperResult = await this.tecommonService.mapper(proRequest, proResponse,proMapData)
              m=1;
              var mapperResponse = {"Mapper Result":mapperResult};              
            }  
            if(mapper.pst.mapData){
              ...
              let mapperResult = await this.tecommonService.mapper(pstRequest, pstResponse,pstMapData )
              m=1;
              var mapperResponse = {"Mapper Result":mapperResult};         
            }     
          }
  ...
  */
  async getdebugNodeProcess(sfnodeDetails,key,upId,nodeName,nodeType,nodeId, params) { 
    this.logger.log("Node level Debug started")
      try {   
        var nodeSjson = await this.tecommonService.getNodeSecurityJson(sfnodeDetails,nodeName)
        ...
        if (nodeid == pfjson[i].nodeId && pfjson[i].nodeType == 'decisionnode' && (pfjson[i].nodeType != 'startnode' || pfjson[i].nodeType != 'endnode')) {    
          try {  
            ...
            if(custconf){            
              if(custconf.request.code){
                var codedata = custconf.request.code
                var customcoderesult = await this.tecommonService.customCodeProcess(key,codedata,arr);
                c=1
                var customCodeResponse = {"CustomCodeResult":customcoderesult} 
              }
            }           
