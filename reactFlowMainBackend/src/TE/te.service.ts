...
export class TeService { 
    ...
    try {
      ...
      if(grpInfo.length == 0){
        ...
        if(result == 'Success'){
          ...
          this.logger.log("PE Stream completed")
          return await this.commonService.responseData(201,result.url)
        }  
        else{
          return result
        } 
  ...
  async staticCodeExec(sfkey,key,upId,nodeId,nodeName,reqdata,token,mode){
    ...
    var exception = await this.commonService.commonErrorLogs('TE',token,key,err,err.status)
    throw exception
  }
  }
  ...
  async Processor(key,upId,nodeId,arr,nodeDetails) {
    ...
    for (var i = 0; i < pfjson.length; i++) {  
      ...
      if (nodeid == pfjson[i].nodeId && pfjson[i].nodeType == 'humantasknode' && (pfjson[i].nodeType != 'startnode' || pfjson[i].nodeType != 'endnode')) { 
        var nodeSjson = await this.tecommonService.getNodeSecurityJson(nodeDetails,pfjson[i].nodeName)        
        if(nodeSjson == true){        
          try{
          this.logger.log("Humantask node started...")      
            ...
            await this.tecommonService.getPElogs(key, upId, pfjson[i].nodeName,pfjson[i].nodeId,pfjson[i].nodeType,fdataReq,fdataRes)
          ...
          {          
            ...
            var exception = await this.tecommonService.getExceptionlogs(error,error.status,pfjson[i].nodeName,pfjson[i].nodeId,arr,key,upId,'NPC','PRO')
            throw new BadRequestException(exception)
          }
        }else{
          //logging Security Exception in TPEExceptionlogs
          var secException = await this.tecommonService.getsecurityExceptionlogs(pfjson[i].nodeName,pfjson[i].nodeId,key,upId)
      ...
      if (nodeid == pfjson[i].nodeId && pfjson[i].nodeType == 'decisionnode' && (pfjson[i].nodeType != 'startnode' || pfjson[i].nodeType != 'endnode')) { 
        ...
        if(nodeSjson == true){       
          try{  
              ...
              if(ruleChk){
                var zenresult = await this.tecommonService.zenrule(key,ruleChk,pfjson[i].nodeId)               
              }  
              var c=0;
              ...
              if(custconf){            
                if(custconf.request.code){
                  ...
                  var coderesult = await this.tecommonService.customCodeProcess(key,codedata,arr);
                  c=1;
              ...
              if (mapper) {                    
                ...
                for (var k = 0; k < pfjson[i].routeArray.length; k++) {   
                  ...
                  //logging execution in PElogs
                  await this.tecommonService.getPElogs(key, upId, pfjson[i].nodeName,pfjson[i].nodeId,pfjson[i].nodeType,decirequest,resData)
      ...
      if (pfjson[i].nodeType == 'endnode') { 
        ...
        await this.tecommonService.getPElogs(key, upId, pfjson[i].nodeName,pfjson[i].nodeId,pfjson[i].nodeType)
        break;    
      }
    }    
    console.log(arr);    
  ...
  async nodePreProcess(key,pfjson,upId){
    ...
    var errorobj = await this.commonService.errorobj('TE',error,error.status)    
    ...
    this.logger.log('Node Processor started!');      
      try{  
        ...
        var custconf = JSON.parse(customConfig).customCode 
          ...
          var ruleChk = JSON.parse(currNode).rule
          if(ruleChk){
            var zenresult = await this.tecommonService.zenrule(key,ruleChk,pfjson.nodeId) 
            z=1; 
      ...
      await this.tecommonService.getPElogs(key, upId, pfjson.nodeName,pfjson.nodeId,pfjson.nodeType,req,apiresult)
      }catch(error){   
        //logging Technical Exception in TPEExceptionlogs
        var exception = await this.tecommonService.getExceptionlogs(error,error.status,pfjson.nodeName,pfjson.nodeId,arr,key,upId,'NPC','PRO')
        throw new BadRequestException(exception)
      }      
  }
        ...
        if (pfjson[i].nodeType == 'humantasknode') {
          ...
          if(nodeSjson == true){
            ...
            if (Object.keys(req).length == 0) {
              ...
              var errorobj = await this.commonService.errorobj('TE','Permission Restricted to Execute the '+pfjson[i].nodeName,403)  
    ...
    try {           
      ...
      if(flag == 'E') {
        ...
        var formjson = await this.returnformdata(key, upId, sjson['Node'])
          ...
          if (formjson == 'Success') {         
            var pfresponse = await this.pfProcessor(key, upId, sjson['Node'],decoded);
  ...
  */
  ...
  /* Process flow Execution by iterating through the all the nodes 
     ...
     @param key - The key used to identify the process flow.  
  */
  async pfProcessor(key, upId, nodeDetails, token) {
    this.logger.log('Pf Processor started!');
      ...
      for (var i = 0; i < pfjson.length; i++) {
        ...
        if (nodeid == pfjson[i].nodeId && pfjson[i].nodeType == 'humantasknode' && (pfjson[i].nodeType != 'startnode' || pfjson[i].nodeType != 'endnode')) {
          ...
          if(nodeSjson == true){
            var nodeprop = await this.redisService.getJsonDataWithPath(key + 'nodeProperty', '.'+pfjson[i].nodeId);
            var notification = JSON.parse(nodeprop).notification
            console.log(notification);
            var msg =  pfjson[i].nodeName +' started'
            if(notification.sms == "Y"){
             await this.commonService.sendSms(loginId,email,firstName,phone,msg)
          }
            if(notification.email == "Y"){
              await this.commonService.sendEmail(loginId,email,firstName,phone,msg)
            }
            try {
              this.logger.log("Humantask node started...")
              var obj = {};
              ...
              await this.tecommonService.getPElogs(key, upId, pfjson[i].nodeName,pfjson[i].nodeId,pfjson[i].nodeType,'NPC','PRO')
            ...
            try {
              ...
              if (mapper) {    
                ...
                if(Object.keys(mapper.ph[0].mapData).length > 0){
                  var mapperconfig = JSON.parse(currNode).mapper.pre
                  var mapperResult = await this.tecommonService.getMapper(mapperconfig) 
                  m = 1;                
                }         
                if(Object.keys(mapper.pro.mapData ).length > 0){
                  var mapperconfig = JSON.parse(currNode).mapper.pro
 ...
 */
  async postapicall(url, params, key, nodeName, upId) {   
    try {
      ...
      const postres = await this.commonService.postCall(url,params)   
      return postres
    } catch (error) {   
